// AWBix Portal — sidebar nav, theme switcher, toast, dynamic menu, track form

(function () {
	"use strict";

	// ── Icon SVG paths (keyed by Portal Menu Item icon value) ─────────────────
	const ICONS = {
	...
	function applyTheme(t) {
		if (!VALID_THEMES.includes(t)) t = "ocean";
	...
	// ── Dynamic menu (loaded from Portal Menu Item DocType) ───────────────────

	const MENU_CACHE_KEY = "awbix_portal_menu";
	const MENU_CACHE_TTL = 5 * 60 * 1000; // 5 min
	...
	async function loadDynamicMenu() {
		if (frappe.is_guest) return;

		// Show cached immediately
		const cached = (() => {
			try {
				const c = JSON.parse(localStorage.getItem(MENU_CACHE_KEY) || "null");
				if (c && Date.now() - c.ts < MENU_CACHE_TTL) return c.items;
			} catch (_) {}
			return null;
		})();
		if (cached) applyDynamicMenu(cached);
	...
	function applyDynamicMenu(items) {
		...
		// Check if admin: add "Configure Menu" at the end
		const adminHtml = `
	...
	// ── Admin section (shown even when menu is static) ────────────────────────

	function initAdminNav() {
		const slot = document.getElementById("pt-nav-admin");
		if (!slot || !frappe.is_admin) return;
		slot.innerHTML = `
			<div class="pt-sidebar__group-label" style="margin-top:.5rem">Admin</div>
			<a href="/portal/menu-config" class="pt-sidebar__link">
				<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
	...
	document.addEventListener("DOMContentLoaded", () => {
		...
		initAdminNav();
		initTrackForm();
		loadDynamicMenu(); // async, updates nav in background
	});
})();
